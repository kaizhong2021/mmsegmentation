...
class LPSNet(BaseDecodeHead):
    ...
    def __init__(
        ...
        if self.num_blocks != len(self.channels_eb):
            raise ValueError(
        ...
        self.head = nn.Conv2D(
            channels_eb[-1] * self.num_paths, num_classes, 1, bias_attr=True)
    ...
    def _build_path(self):
        ...
        for b, (d, c) in enumerate(zip(self.depths, self.channels_eb)):
            blocks = []
            for i in range(d):
                blocks.append(
                    layers.ConvBNReLU(
